#!/usr/bin/env python3
"""Reject unexpected axioms in the machine-checked adapter properties."""
...
result = subprocess.run(
    ["lake", "build", "GripDiagnostics.Properties"],
    text=True,
    capture_output=True,
)
...
for line in (result.stdout + result.stderr).splitlines():
    match = re.search(r"'GripDiagnostics\.Properties\.([^']+)' depends on axioms:", line)
    if match:
